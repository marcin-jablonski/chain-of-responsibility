...
{
    public class HashProcessor : ExpressionProcessor
    {
        protected override void Calculate(Expression expression)
        {
            ...
            {
                expression.Result = int.Parse(expression.Items[0]) * int.Parse(expression.Items[1]) + int.Parse(expression.Items[1]);
            }
        }
    }
}